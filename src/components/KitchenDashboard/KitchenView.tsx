...
import { Order, OrderProgressStatus, DeliveryMode, Theater } from '../../types';
...
export const KitchenView: React.FC<KitchenViewProps> = ({
  ...
  useEffect(() => {
    const updateList = () => {
      setOrders(orderStore.getOrders(currentTheater.theater_id));
    };
    updateList();
  ...
  const handleClearOrders = () => {
    if (typeof window !== 'undefined' && window.confirm('Are you sure you want to clear all orders? Today’s token numbering will start fresh at Token #1.')) {
      ...
      <div className="max-w-7xl mx-auto space-y-5">
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-3.5 shadow-md flex flex-col justify-between">
            ...
            <div className="flex items-center gap-2 mt-1">
              <button
                ...
                className="flex-1 text-[11px] px-2.5 py-1.5 rounded-xl bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 text-neutral-300 font-medium transition-all flex items-center justify-center gap-1 cursor-pointer"
                title="Clear order queue and start fresh for today at Token #1"
              >
        ...
        {filteredOrders.length === 0 ? (
          <div className="py-20 text-center text-neutral-500 space-y-3 bg-neutral-900/40 rounded-3xl border border-neutral-900">
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-emerald-400/90 font-medium pt-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>KDS Live Queue Active • Waiting for incoming in-seat orders</span>
            </div>
          </div>
        ) : (
