...
export const CustomerView: React.FC<CustomerViewProps> = ({
  ...
  useEffect(() => {
    // Immediate fetch on mount or theater switch
    menuStore.fetchMenuFromServer(currentSeat.theater_id).then((items) => {
      setMenuList(items);
    });
    ...
    // Periodic background sync every 8 seconds so QR scanned mobile phones see real-time price updates
    const syncInterval = setInterval(() => {
      menuStore.fetchMenuFromServer(currentSeat.theater_id);
    }, 8000);
