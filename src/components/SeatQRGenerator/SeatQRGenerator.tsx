...
export const SeatQRGenerator: React.FC<SeatQRGeneratorProps> = ({
  ...
  const [isCustomDomainActive, setIsCustomDomainActive] = useState<boolean>(false);

  // Compute live absolute target URL for customer ordering
  // Always targets root '/' customer portal so QR scanning directly opens customer ordering view
  const detectedOrigin = typeof window !== 'undefined' 
    ? window.location.origin
    : 'https://snackbox.n4x.in';

  const cleanBaseUrl = (isCustomDomainActive && customDomain.trim()
    ? customDomain.trim()
    : detectedOrigin).replace(/\/+$/, '');

  const buildCustomerSeatUrl = (screen: string, row: string, seat: string | number) => {
  ...
  const singleStickerUrl = buildCustomerSeatUrl(auditoriumName, singleRow, singleSeat);
  ...
  const handleDownloadAllBatchLabels = () => {
    ...
    currentActiveBatchRows.forEach((row) => {
      for (let s = 1; s <= batchSeatsPerRow; s++) {
        const url = buildCustomerSeatUrl(auditoriumName, row, s);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-neutral-900/90 border border-neutral-800 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-lg">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto text-[11px] self-end sm:self-center">
            ...
            {(theater?.screens?.length ? theater.screens.map((s) => s.name) : DEFAULT_AUDITORIUMS).slice(0, 4).map((p) => (
              <button
                key={p}
        ...
        {/* Production Domain / Vercel URL Bar */}
        <div className="bg-neutral-900/60 border border-neutral-800/80 rounded-2xl p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 text-neutral-300">
            <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0"></span>
            <span className="text-neutral-400">Target QR Domain:</span>
            <strong className="font-mono text-amber-300 truncate max-w-xs">{cleanBaseUrl}</strong>
          </div>
            ...
            {isCustomDomainActive ? (
              <div className="flex items-center gap-1.5 w-full sm:w-80">
                <input
                  ...
                  className="bg-neutral-950 border border-neutral-700 rounded-lg px-2.5 py-1 text-xs text-white placeholder:text-neutral-600 focus:outline-none focus:border-amber-400 w-full"
                />
                <button
                  onClick={() => setIsCustomDomainActive(false)}
                  className="px-2 py-1 text-[11px] text-neutral-400 hover:text-white shrink-0"
                >
                  Reset
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 max-h-[480px] overflow-y-auto p-3 bg-neutral-950 rounded-2xl border border-neutral-800">
                {currentActiveBatchRows.map((row) =>
                  Array.from({ length: batchSeatsPerRow }, (_, i) => i + 1).map((seatNum) => {
                    const url = buildCustomerSeatUrl(auditoriumName, row, seatNum);
                    return (
                      <div
                        key={`${row}-${seatNum}`}
