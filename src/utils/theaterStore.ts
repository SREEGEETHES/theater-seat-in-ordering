...
class TheaterStore {
    ...
    try {
      ...
      if (storedTheaters) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Backfill admin_credentials & theater_owner_email and ensure active real UPI VPA
          this.theaters = parsed.map((t) => {
            const initial = INITIAL_THEATERS.find((init) => init.theater_id === t.theater_id);
            const needsVpaUpdate = !t.kyc?.payee_vpa || t.kyc.payee_vpa.includes('grandcineplex.fnb') || t.kyc.payee_vpa.includes('snackbox.upi') || t.kyc.payee_vpa.includes('cinestar.cp');
            return {
              ...t,
              ...
              kyc: {
                ...t.kyc,
                payee_vpa: needsVpaUpdate ? 'jaspritsreea-1@okaxis' : t.kyc.payee_vpa,
  ...
  public deleteTheater(theaterId: string): boolean {
    if (this.theaters.length <= 1) {
      return false; // Cannot delete the only remaining theater
    }
    const prevCount = this.theaters.length;
    this.theaters = this.theaters.filter((t) => t.theater_id !== theaterId);
    if (this.theaters.length < prevCount) {
      if (this.activeTheaterId === theaterId) {
        this.activeTheaterId = this.theaters[0].theater_id;
        localStorage.setItem(ACTIVE_THEATER_KEY, this.activeTheaterId);
      }
      this.saveTheaters();
      this.notify();
